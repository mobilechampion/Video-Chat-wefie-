...
@interface MHGalleryImageViewerViewController()<AAShareBubblesDelegate, MFMailComposeViewControllerDelegate, CLImageEditorDelegate>
...
-(void)donePressed{
    ...
    if (galleryViewController.finishedCallback) {
        ...
        [galleryViewController.galleryDelegate setEditImage:item.image editShowIndex:self.pageIndex];
...
-(void)viewDidLoad{
    ...
    self.shareBarButton.tintColor = [UIColor whiteColor];
    ...
    self.toolbar.barTintColor = [UIColor colorWithRed:40.0/255.0 green:93.0/255.0 blue:172/255.0 alpha:0.1];
...
-(void)editPressed{
    MHGalleryItem *item = [self itemForIndex:self.pageIndex];
    AlbumDetailViewController *albumEditView = [[AlbumDetailViewController alloc] initWithImage:item.image];
    albumEditView.delegate = self;
    [self.navigationController pushViewController:albumEditView animated:YES];
}
...
-(void)updateToolBarForItem:(MHGalleryItem*)item{
    ...
    if (item.galleryType == MHGalleryTypeVideo) {
        ...
        self.toolbar.items =@[self.shareBarButton,flex,self.leftBarButton,flex,self.rightBarButton,flex,self.editBarButton];
    }
}
        ...
        case AAShareBubbleTypeFacebook:
        {
            ...
            if ([SLComposeViewController isAvailableForServiceType:SLServiceTypeFacebook]) {
                ...
                if (composeViewController) {
                    MHGalleryItem *item = [self itemForIndex:self.pageIndex];
                    [composeViewController addImage:item.image];
...
-(void)viewDidAppear:(BOOL)animated{
    ...
    __weak typeof(self) weakSelf = self;
/*****************************  Mine  *************************************/
    MHGalleryItem *item = [self.viewController itemForIndex:self.pageIndex];
